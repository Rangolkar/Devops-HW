...
class DigitalOceanProvider
{
	...
	async listImages( )
	{
		...
		let response = await got('https://api.digitalocean.com/v2/images/?type=distribution', { headers: headers, responseType: 'json' }).catch(err => console.error(`listImages ${err}`));
	...
	async dropletInfo (id)
	{
		...
		if( response.body.droplet )
		{
			...
			console.log(`Public IP of droplet ${id} is:`)
			for( let access of droplet.networks.v4)
			{
				if (access.type == "public")
				{
					console.log(access.ip_address)
				}
			}
		}
